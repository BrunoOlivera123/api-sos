...
async function register(req, res, tipo) {
  ...
  if (
    tipo === "PRESTADOR" &&
    !(await prisma.categoria.findUnique({ where: { id: Number(categoriaId) } }))
  ) {
    return res
      .status(400)
  ...
  const user = await Model.create({
    data: {
      ...
      telefone,
      cpf,
      ...(tipo === "PRESTADOR" ? { categoriaId: Number(categoriaId) } : {}),
    },
  });
  res
    .status(201)
    .json({
      ...
      data: publicUser(user, tipo),
    });
}
exports.registerCliente = (req, res, next) =>
  register(req, res, "CLIENTE").catch(next);
exports.registerPrestador = (req, res, next) =>
  register(req, res, "PRESTADOR").catch(next);
