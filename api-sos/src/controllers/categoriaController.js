...
exports.list = async (req, res, next) => {
  try {
    res.json({
      success: true,
      data: await prisma.categoria.findMany({ orderBy: { nome: "asc" } }),
    });
  } catch (e) {
    next(e);
  }
...
exports.get = async (req, res, next) => {
  try {
    const d = await prisma.categoria.findUnique({
...
exports.create = async (req, res, next) => {
  try {
    const d = await prisma.categoria.create({ data: { nome: req.body.nome } });
...
exports.update = async (req, res, next) => {
  try {
    const d = await prisma.categoria.update({
      where: { id: Number(req.params.id) },
...
exports.remove = async (req, res, next) => {
  try {
    ...
    const [p, c] = await Promise.all([
      prisma.prestador.count({ where: { categoriaId: id } }),
    ...
    if (p || c)
      return res
        ...
        .json({
          success: false,
          message: "Categoria possui prestadores ou chamados relacionados",
        });
