...
exports.me = async (req, res, next) => {
  try {
    const u = await prisma.prestador.findUnique({
    ...
    if (!u)
      return res
        .status(404)
        .json({ success: false, message: "Prestador não encontrado" });
...
exports.update = async (req, res, next) => {
  try {
    ...
    const u = await prisma.prestador.update({
      where: { id: req.user.id },
      data,
    });
    res.json({ success: true, data: safe(u) });
...
exports.disponiveis = async (req, res, next) => {
  try {
    const users = await prisma.prestador.findMany({
      where: {
        categoriaId: Number(req.query.categoriaId),
        disponivel: true,
        latitude: { not: null },
        longitude: { not: null },
      },
